...
class SupConLoss(nn.Module):
    ...
    def forward(self, features, labels=None, mask=None, weight=None):
        batch_size = features.shape[0]      # label 的个数, 28
        ...
        if labels is not None and mask is not None:
            ...
            mask = (mask > 0).float().to(features.device)       # (28, 28)
        ...
        # compute logits
        anchor_dot_contrast = torch.div(torch.mm(contrast_feature, anchor_feature.T), self.temperature)

        logits_mask = torch.scatter(
            torch.ones_like(mask).to(features.device), 1,
            torch.arange(batch_size).view(-1, 1).to(features.device), 0).to(features.device)
